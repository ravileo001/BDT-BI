package com.sci.testamigo.bi.exceptions;
...
import com.sci.testamigo.bi.screenshots.ScreenShotManager;
      ...
      if (scenarioDetails.getScenarioName().trim().equals(Constants.SCEANRIO_NAME)) {
        for (int j = 0; j < scenarioDetails.getStepDetails().size(); j++) {
          ...
          if (stepName.contains("\"")) {
            verifyStepName = stepName.substring(stepName.indexOf(" "), stepName.indexOf("\""));
          } else {
            verifyStepName = stepName.substring(stepName.indexOf(" "));
          }
          if (verifyStepName.toLowerCase().contains(probleminStepName.toLowerCase())) {
            ...
            statusFlag = 1;
          } else if ((!verifyStepName.contains(probleminStepName)) && statusFlag == 1) {
            break;
          } else {
